...
class Moderation(commands.Cog):
    ...
    async def warn(self, ctx, logChannel: discord.TextChannel = None, user: discord.Member = None, *, reason: str = None):
        ...
        if ctx.me.top_role <= user.top_role:
            return await ctx.send(f"<{ERROR_EMOJI}> **My top role is lower or equal to member's top role, can't warn `{user}`.**")
        if ctx.author.top_role <= user.top_role and ctx.author != ctx.guild.owner:
            return await ctx.send(f"<{ERROR_EMOJI}> **Your top role is lower or equal to member's/Can't warn `{user}`.**")
        if reason is None:
            reason = 'No reason.'
        if logChannel is None:
            return await ctx.send(f"<{ERROR_EMOJI}> **I need a logChannel. [example: #general]**")
        ...
        try:
            ...
            error = await ctx.send(f"<{ERROR_EMOJI}> **`Looks like the user blocked me. DM message failed. Deleting this message in 5 seconds.`** ```py\n{type(e).__name__}: {e}\n```")
            await asyncio.sleep(5)
            await error.delete()
            pass
    ...
    async def add(self, ctx, user: discord.Member, *, reason: str = None):
        ...
        if ctx.me.top_role <= user.top_role:
            return await ctx.send(f"<{ERROR_EMOJI}> **My top role is lower or equal to member's top role, can't mute `{user}`.**")
        if ctx.author.top_role <= user.top_role and ctx.author != ctx.guild.owner:
            return await ctx.send(f"<{ERROR_EMOJI}> **Your top role is lower or equal to member's/Can't mute `{user}`.**")
        if reason is None:
            reason = 'No reason.'
