...
async def _stats(ctx):
    ...
    embed.add_field(name="Usage & Misc", value=f"I am in `{len(bot.guilds)}` servers! CPU Usage is {psutil.cpu_percent()}%\nVirtual Memory: {humanize.naturalsize(psutil.virtual_memory().used)} ({psutil.virtual_memory().percent}%)\nSwap memory: {humanize.naturalsize(psutil.swap_memory().used)} - ({psutil.swap_memory().percent}%)", inline=True)
    embed.add_field(name="Uptime & Ping latency", value=f"I have been up to `{days}d, {hours}h, {minutes}m, {seconds}s` and my websocket latency is `{round(bot.latency * 1000)}ms` and time is `{time_delta}ms.`!\nOS booted since: {datetime.fromtimestamp(psutil.boot_time()).strftime('%Y-%m-%d %H:%M:%S')}.", inline=True)
    embed.add_field(name="Versions", value=f"OS version: {platform.system()}{platform.release()}\nPython Version: {platform.python_version()}\ndiscord.py version: {pkg_resources.get_distribution('discord.py').version}", inline=True)                
